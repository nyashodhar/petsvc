  ...
  def handle_mongoid_validation_error(document, args=nil)
    ...
    if(args.blank?)
      if(document.errors.messages.blank?)
        logger.error "Validation error in doc #{document}, but the document contains no error message, error #{e.inspect}"
        render :status => 422, :json => {:error => I18n.t("422response")}
        return
      else
        logger.error "Validation error for doc: #{document.errors.messages}"
